...
import { getDay, getDaysInMonth, isBefore, isSameDay, isToday, startOfDay } from "date-fns";
...
export const CalendarBody = ({ features, children, onDayClick, maxItemsPerDay = 3 }: CalendarBodyProps) => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      ...
      >
        <span
          className={cn(
            "self-end w-5 h-5 flex items-center justify-center rounded-full leading-none transition-colors",
            isCurrentDay
