		...
		<Popover>
			<PopoverTrigger asChild>
				...
				>
					...
					/>
					<span className="truncate font-medium" style={{ color: feature.status.color }}>
						{feature.name}
					</span>
				</div>
			</PopoverTrigger>
			<PopoverContent className="w-72 p-0 overflow-hidden" align="start">
				<div className="h-1 w-full" style={{ backgroundColor: feature.status.color }} />
				<div className="p-4 space-y-3">
					<div className="space-y-1.5">
						<p className={`text-sm font-medium leading-snug ${task.status === TaskStatus.COMPLETED ? "line-through text-muted-foreground" : "text-foreground"}`}>
							{task.title}
						</p>
				...
				<div>
					...
					>
						<span className="text-muted-foreground/30 group-hover/dur:text-muted-foreground/60 transition-colors">
							{timelineOpen ? <ChevronDownIcon size={10} /> : <ChevronRightIcon size={10} />}
						</span>
						<ClockIcon size={10} className="text-muted-foreground/40 shrink-0" />
...
function DaySheet({ date, tasks, onClose }: DaySheetProps) {
	const totalDuration = tasks.reduce((sum, t) => sum + calculateDuration(t.intervals), 0);
			...
			<SheetContent className="w-full sm:max-w-md flex flex-col gap-0 p-0 overflow-hidden">
				...
				<div className="px-6 pt-5 pb-4 border-b border-border/40 shrink-0">
					...
					<div className="flex items-center gap-2.5">
						<Calendar1Icon size={13} className="text-muted-foreground/50 shrink-0" />
					...
					{/* Stats strip */}
					<div className="flex items-center gap-2.5 mt-3 text-xs pl-[22px]">
						<div className="flex items-center gap-1">
							<span className="font-semibold tabular-nums text-foreground">{tasks.length}</span>
							<span className="text-muted-foreground/60">{tasks.length === 1 ? "task" : "tasks"}</span>
						</div>
						{totalDuration > 0 && (
							<>
			...
			>
				...
				>
					{({ feature }) => {
						...
						if (!task) return null;
						return <TaskCalendarItem key={feature.id} feature={feature} task={task} />;
					}}
				</CalendarBody>
			</CalendarProvider>
